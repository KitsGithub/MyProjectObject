...
- (void)viewDidLoad {
    ...
    // 初始化CollectionView
    self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 64 + 24, SCREEN_WIDTH, SCREEN_HEIGHT - 24 - 64) collectionViewLayout:layout];
...
- (void)setupView {
    ...
    for (int i = 0; i < textArray.count; i++) {
        [self initHeaderWeekText:textArray[i] titleColor:BLACKCOLOR x:(SCREEN_WIDTH / 7) * i y:yOffset];
    }
}

- (void)initHeaderWeekText:(NSString *)text titleColor:(UIColor *)color x:(CGFloat)x y:(CGFloat)y {
    UILabel *titleText = [[UILabel alloc]initWithFrame:CGRectMake(x, y, SCREEN_WIDTH / 7, 24)];
...
- (NSMutableArray *)getMonthArrayOfDays:(int)days showType:(CalendarShowType)type isEnable:(BOOL)isEnable modelArr:(NSArray *)arr
{
    ...
    //返回数据模型数组
    return [self.calendarLogic reloadCalendarView:date selectDate:self.selectedDate needDays:days showType:type isEnable:isEnable priceModelArr:arr isChineseCalendar:self.isDisplayChineseCalendar];
}
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (kind == UICollectionElementKindSectionHeader){
        ...
        monthHeader.masterLabel.text = [NSString stringWithFormat:@"%lu-%lu",(unsigned long)model.year,(unsigned long)model.month];//@"日期";

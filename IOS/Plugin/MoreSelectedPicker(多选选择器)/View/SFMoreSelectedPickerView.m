...
@interface SFMoreSelectedPickerView () 
...
@implementation SFMoreSelectedPickerView{
    UIButton *_bjButton;
    UIView *_contentView;
    ...
    SFMoreSelectedButtonView *_buttonView;
...
- (void)setupView {
    ...
    _buttonView = [[SFMoreSelectedButtonView alloc] init];
    _buttonView.buttonPadding = (SCREEN_WIDTH - 76 * 3) / 4;
...
- (void)bjButtonDidClick:(UIButton *)sender {
    if ([self.delegate respondsToSelector:@selector(SFMoreSelectedPickerViewDidSelectedCancel:)]) {
        [self.delegate SFMoreSelectedPickerViewDidSelectedCancel:self];
    }
    [self hiddenAnimation];
}
- (void)commitButtonDidClick:(UIButton *)sender {
    if ([self.delegate respondsToSelector:@selector(SFMoreSelectedPickerView:commitDidSelected:)]) {
        NSArray *array = [_buttonView getSelectedIndex];
        [self.delegate SFMoreSelectedPickerView:self commitDidSelected:array];
    }
    [self hiddenAnimation];
}
...
- (void)setResourceArray:(NSArray<NSString *> *)resourceArray {
    ...
    CGFloat buttonViewHieght = ceilf(_resourceArray.count / 3.0) * (36 + 14);
...
- (void)layoutSubviews {
    if (hasSubView) {
        ...
        CGFloat buttonViewHieght = ceilf(self.resourceArray.count / 3.0) * (36 + 14);
